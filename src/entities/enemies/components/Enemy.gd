...
func setup(enemy_data: EnemyData) -> void:
	...
	# Aktywacja emiterów cząsteczek (silniki, smugi, ogień)
...
func setup_intangible_state() -> void:
	# 1. Wyłączamy warstwy i maski kolizji fizycznej
	collision_layer = 0
	collision_mask = 0

	# 2. Wyłączamy detekcję na wszystkich podrzędnych Area2D (Hurtbox / Hitbox)
	for child in get_children():
		if child is Area2D:
...
func start_escape_sequence() -> void:
	...
	# Wyłączamy fizykę podczas ucieczki
	...
	# Odczekujemy chwilę przed usunięciem (np. na dokończenie smug cząsteczek)
...
func _on_area_2d_area_entered(area: Area2D) -> void:
	# Byt niematerialny lub w trakcie ucieczki nie wchodzi w interakcje
